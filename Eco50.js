...
let item_option = ['หน้าไม้ที่จะใช้ 20 มม.','หน้าไม้ที่จะใช้ 50 มม.'];
let select = "";

let select_optioninstall = document.getElementById('optioninstall');
let item_optioninstall = ['ระยะชนชิด','เว้นร่อง 10 มม.','เว้นร่อง 20 มม.','เว้นร่อง 30 มม.','เว้นร่อง 40 มม.','เว้นร่อง 50 มม.'];
let selectinstall = "";
...
function sumitem(option_item,option_install,width,length){
    ...
    }else if(option_item === "หน้าไม้ที่จะใช้ 50 มม."){

        if(option_install === "ระยะชนชิด"){
            total_item = Math.ceil(area) * 6.66;
            return total_item;
        }else if(option_install === "เว้นร่อง 10 มม."){
            total_item = Math.ceil(area) * 5.56;
            return total_item;    
        }else if(option_install === "เว้นร่อง 20 มม."){
            total_item = Math.ceil(area) * 4.76;
            return total_item;    
...
button_sam.addEventListener('click', () => {
    ...
    price_amount = Math.ceil(sum_item) * 260;
